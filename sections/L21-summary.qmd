...
1. If we have a standard "less than" problem, so want to know $\mathbb P(Z \leq z) = \Phi(z)$, we can just look this up in the table.
2. If we have a "greater than", so want to know $\mathbb P(Z > z)$, we can use the complement rule to write $\mathbb P(Z > z) = 1 - \mathbb P(Z \leq z) = 1 - \Phi(z)$. We then look up $\Phi(z)$ in the table, and our answer is $1 - \Phi(z)$.
3. If we have a negative number, we can use the fact that the normal distribution is symmetric about 0. So we swap to the positive value of the number, and switch the inequality. So
\begin{align*}
